...
class App extends React.Component {
	...
	imageSelection(id) {
		...
		this.setState({
			selected_meme: Object.values(this.state.memes).find(
				(meme) => Number(meme.id) == Number(id)
			),
		})
	...
	render() {
		...
		return (
			<div className='App'>
				<Caption
					meme={this.state.selected_meme}
					requestMeme={this.requestMeme}
				/>
				<SelectedImage
					meme={this.state.selected_meme}
				...
				<Gallery
					...
					clickHandler={this.imageSelection}
				/>
			</div>
		)
	}
